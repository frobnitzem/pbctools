...
\begin{abstract}
  The VMD plugin ``\pbctools'' provides procedures to handle periodic
  boundary conditions, \ie to set and get the unitcell parameters, to
  wrap atoms into the central image, to unwrap atoms when they have
  been wrapped, and to draw the unit cell vectors. All of the
  procedures are able to handle non-orthorhombic periodic boundary
  conditions and changes of the unitcell parameters over time (as for
  example in constant pressure simulations).
\end{abstract}
...
\section{Basic usage and command summary}
...
The subcommands that can be used are summarized in table
\ref{tab:subcommands}.

\begin{table}[htb]
  \centering
  \begin{tabular}{|p{0.35\textwidth}|p{0.55\textwidth}|r|}
    \hline
    ...
    & Unwrap frames of a trajectory, so that no atoms are wrapped
    between adjacent frames.
    & \pageref{sec:unwrap}
    \\\hline
    
    \texttt{join} \textit{compound} [\textit{options}\dots]
    & Joins compounds (residues, chains, segments, or fragments) that
    have been split due to wrapping around the unit cell boundaries,
    so that they are not split anymore.
    & \pageref{sec:join}
    \\\hline
    
    \texttt{box} [\textit{options}\dots]
    & (Re)Draws a box that shows the boundaries of the unit cell. The
    box will automatically adapt to changes in the unit cell
    parameters in the course of a trajectory.
    & \pageref{sec:box}
    \\\hline

    \texttt{box\_draw} [\textit{options}\dots]
    & Draws a static box that shows the boundaries of the unit cell,
    but will not adapt to changes in the unitcell properties.
    & \pageref{sec:box_draw}
    \\\hline
  \end{tabular}
  \caption{Subcommands of the VMD-Tcl-command \texttt{pbc}.}
...
\texttt{-center} \texttt{origin}$|$\texttt{unitcell}$|$\textit{sel}
& Set the center of the wrapping cell to the origin, to the center of
the unit cell or to the center of the enclosing box of a given
selection \textit{sel} (default: \texttt{unitcell}).
\\ \hline
...
\texttt{-center} \texttt{origin}$|$\texttt{unitcell}$|$\textit{sel}
& Set the center of the box to the origin, to the center of the unit
cell or to the center of the enclosing box of the given selection
\textit{sel}. (default: \texttt{unitcell})
\\ \hline
